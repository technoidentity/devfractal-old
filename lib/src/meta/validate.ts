...
function errorsForNumberRefinements(
  ...
  if (r.positive && !validators.positive(value)) {
    errors.push(errorMessages.positive)
  }
  if (r.negative && !validators.negative(value)) {
...
function errorsForStringRefinements(
  ...
  if (r.email && !validators.email(value)) {
    errors.push(errorMessages.email)
  }

  if (r.url && !validators.url(value)) {
    errors.push(errorMessages.url)
  }

  if (r.lowercase && !validators.lowercase(value)) {
    errors.push(errorMessages.lowercase)
  }

  if (r.uppercase && !validators.uppercase(value)) {
    errors.push(errorMessages.uppercase)
  }

  if (
    r.maxStringLength &&
    !validators.maxStringLength(r.maxStringLength)(value)
  ) {
    errors.push(errorMessages.maxStringLength(r.maxStringLength))
  }

  if (
    r.minStringLength &&
    !validators.minStringLength(r.minStringLength)(value)
  ) {
    errors.push(errorMessages.minStringLength(r.minStringLength))
  }

  if (r.length && !validators.length(r.length)(value)) {
    errors.push(errorMessages.length(r.length))
  }
