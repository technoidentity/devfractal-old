...
export const Min = (min: number) => t.refinement(t.Number, n => n >= min, 'Min')
export const Max = (max: number) => t.refinement(t.Number, n => n <= max, 'Max')
...
export const MinInt = (min: number) =>
  t.refinement(t.Integer, n => n >= min, 'MinInt')
export const MaxInt = (max: number) =>
  t.refinement(t.Integer, n => n <= max, 'MaxInt')
...
export const Length = (len: number) =>
  t.refinement(t.String, s => s.length === len, 'StringLength')
export const MinStringLength = (min: number) =>
  t.refinement(t.String, s => s.length >= min, 'MinStringLength')
export const MaxStringLength = (max: number) =>
  t.refinement(t.String, s => s.length <= max, 'MaxStringLength')

export const MinDate = (min: Date) =>
  t.refinement(t.Date, d => compareAsc(min, d) <= 0, 'MinDate')
export const MaxDate = (max: Date) =>
  t.refinement(t.Date, d => compareAsc(max, d) >= 0, 'MaxDate')

export const ArrayMinLength = (min: number) =>
  t.refinement(t.Array, arr => arr.length >= min, 'ArrayMinLength')
export const ArrayMaxLength = (max: number) =>
  t.refinement(t.Array, arr => arr.length <= max, 'ArrayMaxLength')
